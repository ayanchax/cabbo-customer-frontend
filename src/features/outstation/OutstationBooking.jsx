...
function OutstationBooking({ orderData, bookingData }) {
  ...
  const fetchedHops = bookingData?.preferences?.hops || null;

  const startDate = { isoString: bookingData?.preferences?.start_date || null };
  const endDate = { isoString: bookingData?.preferences?.end_date || null };
  ...
  const fetchedTimezone =
      bookingData?.preferences?.timezone ||
  ...
  const isRoundTripOnly = bookingData?.metadata?.is_round_trip || false;
  
  const totalTripDays = bookingData?.metadata?.total_trip_days || null;
  const includedKms = bookingData?.metadata?.included_kms || null;
